...
function DepartmentNode({
  ...
  const unassignedUsers = departmentUsers.filter(
    (user) =>
      ...
      <CollapsibleTrigger className="flex items-center justify-between w-full p-4 text-left hover:bg-gray-50">
        <div className="flex items-center space-x-3">
          ...
          <Badge variant="outline" className="text-xs bg-primary/5">
            ...
            {departmentUsers.length === 1 ? "member" : "members"}
          </Badge>
        </div>
          ...
          {managers.length > 0 && (
            <div className="space-y-6">
              ...
              {managers.map((manager) => (
                <div
                  key={manager.id}
                  className="pl-4 border-l-2 border-primary/30 rounded"
                >
                  <div
                    ...
                    <div>
                      ...
                      {reportingStructure[manager.id]?.length > 0 && (
                        <div className="text-xs text-gray-400 mt-1">
                          Managing {reportingStructure[manager.id].length}{" "}
                          {reportingStructure[manager.id].length === 1
                            ? "employee"
                            : "employees"}
                        </div>
                      )}
