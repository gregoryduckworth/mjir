...
export function EmployeeCard({
  ...
  const directReports = allUsers.filter(
    (user) => user.managerId === employee.id
  );
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="py-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    {employee.phoneNumber || "Phone number not available"}
                  </span>
                </div>
                {employee.employeeId && (
                  <div className="flex items-center gap-2">
                ...
                <div className="bg-gray-50 p-3 rounded-md mt-4">
                  ...
                  <button
                    ...
                    onClick={() => {
                      onClose();
                      setTimeout(() => {
                        // Open the manager's profile after closing the current one
                        const managerEmployee = allUsers.find(
                          (user) => user.id === manager.id
                        );
                        if (managerEmployee) {
                          // We need to set a timeout to ensure the current dialog closes first
                          // before opening the new one
                          const event = new CustomEvent("openEmployeeProfile", {
                            detail: { employee: managerEmployee },
                          });
          ...
          <Tabs
            defaultValue={canViewPersonalDetails ? "personal" : "employment"}
            className="w-full"
          >
            <TabsList
              className={`grid ${
                ...
                canViewSkills
                  ...
                  : canViewPersonalDetails &&
                    canViewEmploymentDetails &&
                    (canViewCompensation || canViewSkills)
                  ? "grid-cols-3"
                  : "grid-cols-2"
              } mb-4`}
            >
