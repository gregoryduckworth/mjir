...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/dashboard/stats", async (req, res) => {
    ...
    const pendingRequests = holidayRequests.filter(r => r.status === "pending").length;
    ...
    const learningCompletion = courses.length > 0 
    ...
    const onHoliday = departmentHolidays.flat().filter(h => 
      h.status === "approved" &&
      new Date(h.startDate) <= new Date() &&
      new Date(h.endDate) >= new Date()
    ).length;
    
    const teamAvailability = departmentUsers.length > 0
      ? Math.round(((departmentUsers.length - onHoliday) / departmentUsers.length) * 100)
      : 100;
  ...
  app.get("/api/holiday-requests/upcoming", async (req, res) => {
    ...
    const upcomingRequests = allRequests
      .filter(r => r.status !== "rejected" && new Date(r.startDate) >= new Date())
  ...
  app.get("/api/holiday-requests/balance", async (req, res) => {
    ...
    const totalUsed = approvedRequests.reduce((acc, req) => acc + req.duration, 0);
  ...
  app.post("/api/holiday-requests", async (req, res) => {
    ...
    try {
      const data = insertHolidayRequestSchema.parse(req.body);
  ...
  app.get("/api/learning/stats", async (req, res) => {
    ...
    const totalCompletionRate = totalCourses > 0 
      ? Math.round((completedCourses / totalCourses) * 100) 
      : 0;
    
    // Calculate completion by category
    const completedByCategory: Record<string, { completed: number; total: number }> = {};
    ...
    courses.forEach(course => {
      ...
      const isCompleted = progress.some(p => p.courseId === course.id && p.isCompleted);
      if (isCompleted) {
